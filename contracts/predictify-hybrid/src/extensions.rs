use soroban_sdk::{
    contracttype, token, Address, Env, Map, String, Symbol, Vec, vec, symbol_short,
};
...
impl ExtensionManager {
    ...
    pub fn extend_market_duration(
        ...
        let extension = MarketExtension::new(env, additional_days, admin.clone(), reason, fee_amount);
    ...
    pub fn get_market_extension_history(env: &Env, market_id: Symbol) -> Result<Vec<MarketExtension>, Error> {
        let market = MarketStateManager::get_market(env, &market_id)?;
        Ok(market.extension_history)
    }
    
    /// Get extension statistics for a market
    pub fn get_extension_stats(env: &Env, market_id: Symbol) -> Result<crate::types::ExtensionStats, Error> {
        let market = MarketStateManager::get_market(env, &market_id)?;
        ...
        Ok(crate::types::ExtensionStats {
            ...
            can_extend: ExtensionValidator::can_extend_market(env, &market_id, &market.admin).is_ok(),
...
impl ExtensionTestHelpers {
    ...
    pub fn simulate_market_extension(
        ...
        ExtensionManager::extend_market_duration(env, admin, market_id.clone(), additional_days, reason)
    }
}
...
mod tests {
    ...
    fn test_extension_validation() {
        ...
        assert!(ExtensionValidator::validate_extension_conditions(&env, &symbol_short!("test"), 5).is_err()); // Market doesn't exist
        
        // Test invalid extension days
        assert_eq!(
            ExtensionValidator::validate_extension_conditions(&env, &symbol_short!("test"), 0).unwrap_err(),
            Error::InvalidExtensionDays
        );
        
        assert_eq!(
            ExtensionValidator::validate_extension_conditions(&env, &symbol_short!("test"), 31).unwrap_err(),
    ...
    fn test_extension_fee_calculation() {
        assert_eq!(ExtensionManager::calculate_extension_fee(1), EXTENSION_FEE_PER_DAY);
        assert_eq!(ExtensionManager::calculate_extension_fee(5), 5 * EXTENSION_FEE_PER_DAY);
        assert_eq!(ExtensionManager::calculate_extension_fee(30), 30 * EXTENSION_FEE_PER_DAY);
    }
