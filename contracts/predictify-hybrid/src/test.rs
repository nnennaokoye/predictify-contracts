...
// ===== TIED OUTCOME PAYOUT TESTS =====

/// Test two-outcome tie with equal stakes
/// Requirements: Test two-outcome tie scenarios
#[test]
fn test_two_outcome_tie_equal_stakes() {
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
...
fn test_edge_case_single_winner_not_tie() {
    ...
    // Winner should be claimed
    let market_after = test.env.as_contract(&test.contract_id, || {
        test.env.storage().persistent().get::<Symbol, Market>(&market_id).unwrap()
    });
    assert!(market_after.claimed.get(winner.clone()).unwrap_or(false));

    // Winner gets full pool (300 XLM) minus fee
    // (100/100) * 300 * 0.98 = 294 XLM
    let winner_balance = test.env.as_contract(&test.contract_id, || {
        storage::BalanceStorage::get_balance(&test.env, &winner, &types::ReflectorAsset::Stellar)
    });
    assert!(winner_balance.amount >= 290_0000000 && winner_balance.amount <= 295_0000000, 
            "Winner balance {} not in expected range", winner_balance.amount);

    // Loser should have 0 balance
    let loser_balance = test.env.as_contract(&test.contract_id, || {
        storage::BalanceStorage::get_balance(&test.env, &loser, &types::ReflectorAsset::Stellar)
    });
    assert_eq!(loser_balance.amount, 0);
}

/// Test edge case: all users vote for same outcome
/// Requirements: Test edge case where all vote same
#[test]
fn test_edge_case_all_same_outcome() {
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
